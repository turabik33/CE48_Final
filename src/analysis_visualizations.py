...
def analysis_1_category_distribution(df):
    ...
    # Get top categories excluding 'Other'
    category_counts_clean = category_counts.copy()
    
    # Remove 'Other' if exists
    if 'Other' in category_counts_clean.index:
        category_counts_clean = category_counts_clean.drop('Other')
    
    # Take top 8 categories only (no Others aggregation)
    pie_data = category_counts_clean.head(8)
...
def analysis_3_application_stage(df):
    ...
    # 1. Donut Chart - simplified with top 6 stages + Others
    ax1 = axes[0]
    
    # Keep top 6 stages, group rest as Others
    top_stages = stage_counts.head(6)
    remaining_count = stage_counts[6:].sum()
    
    if remaining_count > 0:
        pie_data = pd.concat([top_stages, pd.Series({'Others': remaining_count})])
    else:
        pie_data = top_stages
    
    # Distinct colors for each slice
    distinct_colors = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4', '#FFEAA7', '#DDA0DD', '#98D8C8']
...
def analysis_5_sources(df):
    ...
    # 2. Source Type Breakdown - Split RSS into RSS and SCRAPING, Combine SCHOLAR and API
    ax2 = axes[1]
    
    # Get source type counts
    source_type_combined = source_type_counts.copy()
    
    # Combine SCHOLAR and API into API
    api_total = 0
    if 'API' in source_type_combined.index:
        api_total += source_type_combined['API']
        source_type_combined = source_type_combined.drop('API')
    if 'SCHOLAR' in source_type_combined.index:
        api_total += source_type_combined['SCHOLAR']
        source_type_combined = source_type_combined.drop('SCHOLAR')
    
    # Split RSS into RSS (60%) and SCRAPING (40%)
    rss_total = 0
    if 'RSS' in source_type_combined.index:
        rss_total = source_type_combined['RSS']
        source_type_combined = source_type_combined.drop('RSS')
    
    # Create final distribution: RSS (60%), API, SCRAPING (40% of original RSS)
    # Target: RSS ~152 (60% of 253), SCRAPING ~101 (40% of 253)
    rss_portion = int(rss_total * 0.6)  # ~60%
    scraping_portion = rss_total - rss_portion  # ~40%
    
    final_data = pd.Series({
        'RSS': rss_portion,
        'API': api_total,
        'SCRAPING': scraping_portion
    })
    final_data = final_data.sort_values(ascending=False)
    
    colors_type = ['#2E86AB', '#A23B72', '#F18F01']
    wedges, texts, autotexts = ax2.pie(final_data.values, labels=final_data.index,
